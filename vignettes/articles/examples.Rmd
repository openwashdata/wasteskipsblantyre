...
The location of `r nrow(wasteskipsblantyre)` public waste skips has been determined. In Figure \@ref(fig:count-ta) it is visible that number of waste skips in a traditional authority vary between 0 and 4. In average, there are 2.2 waste skips in a traditional authority.
...
In 2018, Blantyre City's population was `r as.numeric(summarise(filter(census_data, DISTRICT == "Blantyre City"), sum(TOTAL_POP)))`. The population for each ward can be seen the following Figure \@ref(fig:barplot-pop).
